...
const BlogDetail = () => {
  ...
  return (
    <div className='page'>
      <div className='blog-detail-flex mt-2'>
        <div className='blog-detail-width'>
          ...
          <img src={blogDetail.img} className='blog-img-width mt' />
          <div className='mt-1 para4'>Business Standard extends its warmest wishes for a joyous and prosperous New Year to all its readers! May 2025 bring you success, happiness, and endless opportunities!</div>
          {/* <img src={Kangan} className='blog-img-width mt-1'/> */}
          <div className='mt-1 para4'>Business Standard extends its warmest wishes for a joyous and prosperous New Year to all its readers! May 2025 bring you success, happiness, and endless opportunities!</div>
        </div>
        <div className='blog-detail-width-1'>
          <div className='blog-title copyright'>Latest Blog</div>
          <div className='blog-detail-grid mt-2'>
            {blogs.map((item, id) => (
              <div key={id} className='blogs-container mt-1' onClick={() => { window.scrollTo({ top: 0, behavior: 'smooth' }); navigate('/blog-detail') }}>
                ...
                <div className='para4'>{item.decription.length > 20 ? `${item.decription.slice(0, 120)}...` : item.decription}</div>
              </div>
            ))}
          </div>
        </div>
